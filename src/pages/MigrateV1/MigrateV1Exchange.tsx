    ...
    <>
      <AutoRow style={{ justifyContent: 'flex-start', width: 'fit-content' }}>
        ...
        <div style={{ marginLeft: '.75rem' }}>
          <TYPE.mediumHeader>
            ...
            {chainId && token.equals(WETH[chainId]) ? 'WBTC' : token.symbol}/BTC
          </TYPE.mediumHeader>
        </div>
      </AutoRow>
      ...
      <RowBetween my="1rem">
        <Text fontSize={16} fontWeight={500}>
          Pooled {chainId && token.equals(WETH[chainId]) ? 'WBTC' : token.symbol}:
        </Text>
        <RowFixed>
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        ...
        {chainId && (
          <ExternalLink href={getEtherscanLink(chainId, MIGRATOR_ADDRESS, 'address')}>
            <TYPE.blue display="inline">OnenessSwap migration contract↗</TYPE.blue>
          </ExternalLink>
        )}
        .
      </TYPE.body>
      ...
      {!isFirstLiquidityProvider && largePriceDifference ? (
        <YellowCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            It{"'"}s best to deposit liquidity into OnenessSwap V2 at a price you believe is correct. If the V2 price
          ...
          <AutoColumn gap="8px">
            <RowBetween>
              ...
              <TYPE.black>
                {v1SpotPrice?.toSignificant(6)} {token.symbol}/BTC
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v1SpotPrice?.invert()?.toSignificant(6)} BTC/{token.symbol}
              </TYPE.black>
            </RowBetween>
            ...
            <RowBetween>
              ...
              <TYPE.black>
                {v2SpotPrice?.toSignificant(6)} {token.symbol}/BTC
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v2SpotPrice?.invert()?.toSignificant(6)} BTC/{token.symbol}
              </TYPE.black>
            </RowBetween>
      ...
      {isFirstLiquidityProvider && (
        <PinkCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            You are the first liquidity provider for this pair on OnenessSwap V2. Your liquidity will be migrated at the
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {`Your OnenessSwap V1 ${token.symbol}/BTC liquidity will become OnenessSwap V2 ${token.symbol}/BTC liquidity.`}
      </TYPE.darkGray>
    </AutoColumn>
  )
}
  ...
  const liquidityToken: Token | undefined = useMemo(
    () =>
      validatedAddress && chainId && token
        ? new Token(chainId, validatedAddress, 18, `UNI-V1-${token.symbol}`, 'OnenessSwap V1')
        : undefined,
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          <div>
            <QuestionHelper text="Migrate your liquidity tokens from OnenessSwap V1 to OnenessSwap V2." />
          </div>
        </AutoRow>
        ...
        {!account ? (
          ...
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              Because OnenessSwap V2 uses WETH under the hood, your OnenessSwap V1 WBTC/BTC liquidity cannot be
