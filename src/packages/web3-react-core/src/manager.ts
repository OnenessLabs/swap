...
const reducerDefault = () => ({ lastActive: getLastActive() })
function reducer(state: Web3ReactManagerState, { type, payload }: Action): Web3ReactManagerState {
  switch (type) {
    case ActionType.ACTIVATE_CONNECTOR: {
      const { connector, lastActive, provider, chainId, account, onError } = payload
      return { connector, lastActive, provider, chainId, account, onError }
    }
...
const lastActiveKey = 'last.connector'
const getLastActive = () => {
  const saved = localStorage.getItem(lastActiveKey) ?? undefined
  return saved
}
const removeLastActive = () => localStorage.removeItem(lastActiveKey)

export function useWeb3ReactManager(): Web3ReactManagerReturn {
  const [state, dispatch] = useReducer(reducer, reducerDefault())
  const { connector, lastActive, provider, chainId, account, onError, error } = state
  ...
  const activate = useCallback(
    async (
      ...
      try {
        ...
        let _lastActive = 'x'
        if (activated) {
          const _provider = update.provider === undefined ? await connector.getProvider() : update.provider
          if (_provider.isWalletConnect) {
            _lastActive = 'wc'
            localStorage.setItem(lastActiveKey, _lastActive)
          } else removeLastActive()
        }
        dispatch({
          type: ActionType.ACTIVATE_CONNECTOR,
          payload: { connector, ...augmentedUpdate, lastActive: _lastActive, onError }
        })
      } catch (error) {
        removeLastActive()
  ...
  return { connector, provider, chainId, lastActive, account, activate, setError, deactivate, error }
}
