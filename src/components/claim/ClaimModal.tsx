...
const ModalUpper = styled(DataCard)`
  ...
  background: radial-gradient(76.02% 75.41% at 1.84% 0%, #e2c54f 0%, #021d43 100%);
`
      ...
      {!attempting && !claimConfirmed && (
        <ContentWrapper gap="lg">
          <ModalUpper>
            ...
            <CardSection gap="md">
              <RowBetween>
                <TYPE.white fontWeight={500}>Claim OST</TYPE.white>
                <CloseIcon onClick={toggleClaimModal} style={{ zIndex: 99 }} color="white" />
              </RowBetween>
              <TYPE.white fontWeight={700} fontSize={36}>
                {unclaimedAmount?.toFixed(0, { groupSeparator: ',' } ?? '-')} OST
              </TYPE.white>
            </CardSection>
            <Break />
            <CardSection gap="sm">
              {userClaimData?.flags?.isSOCKS && (
                <RowBetween>
                  ...
                  <TYPE.subHeader color="white">{SOCKS_AMOUNT} OST</TYPE.subHeader>
                </RowBetween>
              )}
              ...
              {userClaimData?.flags?.isUser && (
                <RowBetween>
                  ...
                  <TYPE.subHeader color="white">{USER_AMOUNT} OST</TYPE.subHeader>
                </RowBetween>
              )}
            </CardSection>
          </ModalUpper>
          <AutoColumn gap="md" style={{ padding: '1rem', paddingTop: '0' }} justify="center">
            <TYPE.subHeader fontWeight={500}>
              As a member of the OnenessSwap community you may claim OST to be used for voting and governance. <br />{' '}
              <br />
              <ExternalLink href="https://uonenesslabs.io">Read more about OST</ExternalLink>
            </TYPE.subHeader>
            <ButtonPrimary
        ...
        <ConfirmOrLoadingWrapper activeBG={true}>
          ...
          <AutoColumn gap="100px" justify={'center'}>
            <AutoColumn gap="12px" justify={'center'}>
              ...
              {!claimConfirmed && (
                <Text fontSize={36} color={'#E2C54F'} fontWeight={800}>
                  {unclaimedAmount?.toFixed(0, { groupSeparator: ',' } ?? '-')} OST
                </Text>
              )}
            </AutoColumn>
